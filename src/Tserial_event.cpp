...
void Tserial_event_thread_start(void *arg)
{
    ...
    if( ( serial_unit = (Tserial_event *)arg ) != 0 ) 
        serial_unit->run();
}
...
int  Tserial_event::connect (char *port_arg, unsigned  rate_arg, unsigned ByteSize , int parity_arg,
                              int StopNum, bool modem_events)
{
    int  erreur;
    int  i;
    bool hard_handshake;
    ...
    if (port_arg!=0)
    {
        ...
        */
        ...
        /* -------------------------------------------------------------------- */
        serial_handle    = CreateFile(convertCharArrayToLPCWSTR(port), GENERIC_READ | GENERIC_WRITE,
...
/* --------------------------    sendData     ------------------------- */
/* -------------------------------------------------------------------- */
int Tserial_event::sendData (char *buffer, int size)
{
...
void Tserial_event::setRts(void)
{
	EscapeCommFunction( serial_handle, SETRTS );
}

void Tserial_event::resetRts(void)
{
	EscapeCommFunction( serial_handle, CLRRTS );
}

void Tserial_event::setDtr(void)
{
	EscapeCommFunction( serial_handle, SETDTR );
}

void Tserial_event::resetDtr(void)
{
        ...
        else
        {
            ...
            switch(status)
            {
                    ...
                    if (!rx_in_progress)
                    {
                        ...
                        if (!success)
                        {
                            ...
                            if(GetLastError() != ERROR_IO_PENDING )
                            {
                                ...
                                #ifdef DEBUG_EVENTS
                                printf("%u:Readfile error (not pending)\n", (char)this);
                                #endif DEBUG_EVENTS
                            }
                            #ifdef DEBUG_EVENTS
                            else
                                printf("%u:ReadFile pending\n", (char)this);
                            #endif DEBUG_EVENTS
                        }
                        #ifdef DEBUG_EVENTS
                        else
                        {
                            ...
                            printf("%u:ReadFile immediate success\n", (char)this);
                        }
                        #endif
                    }
                    break;
                ...
                case SIG_READER:
                    ...
                    {
                        #ifdef DEBUG_EVENTS
                            printf("%u:ReadFile => GetOverlappedResult done\n", (char)this);
                        #endif DEBUG_EVENTS
                        // no error => OK
                    ...
                    success = (char) WriteFile(serial_handle, txBuffer, tx_size,
                        ...
                        if (!success)
                        {
                            ...
                            if(GetLastError() != ERROR_IO_PENDING )
                            {
                                ...
                                #ifdef DEBUG_EVENTS
                                printf("%u,WriteFile error (not pending)\n", (char)this);
                                #endif DEBUG_EVENTS
                            }
                            #ifdef DEBUG_EVENTS
                            else
                                printf("%u,WriteFile pending\n, (char)this");
                            #endif DEBUG_EVENTS
                        }
                        #ifdef DEBUG_EVENTS
                        else
                        {
                            ...
                            // will be signaled anyway, so I'll make
                            // the processing there
                            printf("%u, WriteFile immediate success\n", (char)this);
                        }
                        #endif
                    break;
                /* ######################################################## */
                    ...
                    {            
                        ...
                        if (!success)
                        {
                            ...
                            if(GetLastError() != ERROR_IO_PENDING )
                            {
                                ...
                                #ifdef DEBUG_EVENTS
                                printf("%u:WaitCommEvent error (not pending)\n", (char)this);
                                #endif DEBUG_EVENTS
                            }
                            #ifdef DEBUG_EVENTS
                            else
                                printf("%u:WaitCommEvent pending\n", (char)this);
                            #endif DEBUG_EVENTS
                        }
                        #ifdef DEBUG_EVENTS
                        else
                        {
                            ...
                            // will be signaled anyway, so I'll make
                            // the processing there
                            printf("%u:WaitCommEvent immediate success\n", (char)this);
                        }
                        #endif
                    }
                    break;
